...
export const getStatistics = api<void, Statistics>(
  ...
  async () => {
    try {
      ...
      return {
        totalSessions: parseInt(totalSessionsRow?.count || '0'),
        totalWorkSessions: parseInt(workSessionsRow?.count || '0'),
        totalBreakSessions: parseInt(breakSessionsRow?.count || '0'),
        totalMinutes: parseInt(totalMinutesRow?.total || '0'),
        completedTasks: parseInt(completedTasksRow?.count || '0'),
        averageSessionsPerDay: Math.round(parseFloat(sessionsPerDayRow?.avg || '0') * 10) / 10,
        streakDays,
      };
    } catch (error) {
