...
export function TaskList() {
  ...
  const getProgressColor = (completed: number, estimated: number): string => {
    ...
    if (percentage >= 100) return 'bg-gradient-to-r from-green-400 to-green-500';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-gradient-to-r from-purple-500/80 to-blue-500/80 hover:from-purple-600/80 hover:to-blue-600/80 backdrop-blur-sm border border-white/20 text-white shadow-lg">
      ...
      {/* Active Tasks */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-white/90">Active Tasks ({activeTasks.length})</h3>
        {activeTasks.length === 0 ? (
          <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl">
            <div className="text-center">
              <p className="text-white/70">No active tasks. Create one to get started!</p>
            </div>
          </div>
        ) : (
          <div className="grid gap-4">
            {activeTasks.map((task) => (
              <div key={task.id} className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl transition-all hover:bg-white/15">
      ...
      {completedTasks.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-white/90">Completed Tasks ({completedTasks.length})</h3>
          <div className="grid gap-4">
            {completedTasks.map((task) => (
              <div key={task.id} className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-2xl opacity-75">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <CheckCircle className="w-5 h-5 text-green-400" />
                      <h4 className="font-semibold text-lg line-through text-white/70">{task.title}</h4>
                      <Badge variant="secondary" className="bg-green-500/20 text-green-300 border-green-400/30">
                        Completed
                      </Badge>
                    </div>
                    {task.description && (
                      <p className="text-white/50 mb-3">{task.description}</p>
                    )}
                    <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => handleDeleteTask(task.id)}
                    className="bg-white/10 hover:bg-white/20 backdrop-blur-sm border-white/20 text-white/70"
                  >
                    <Trash2 className="w-4 h-4" />
                  </Button>
                </div>
